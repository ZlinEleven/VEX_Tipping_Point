...
double LEFT_TRACK = 4.25;  // Left tracking wheel distance from tracking center
double RIGHT_TRACK = 4.25; // right tracking wheel distance from tracking center
double BACK_TRACK = 0;  // back tracking wheel distance from tracking center
...
void getEncValue() {//Tuning encoder ticks to inches. You will have to tune these
  currentL = (encL.rotation(rotationUnits::deg) * inchPerTick);
  currentR = encR.rotation(rotationUnits::deg) * inchPerTick;
